...
class SQVD(object):
    ...
    def login(self,username=None,password=None):
        ...
        self.session.headers.update({ "X-Auth-Token": auth['authToken'], "X-User-Id": auth['userId'] })
        return True
    ...
    def rest(self, collection, op='GET', data=None, json=None):
        ...
        if op == 'GET':
            if isinstance(data, string_types):
                ...
                baseUrl[-1] += '?'+'&'.join(map(lambda k: str(k)+'='+str(data[k]),data.keys()))
            r = self.session.request(op, '/'.join(baseUrl))
        elif op == 'DELETE':
            baseUrl.append(data)
            r = self.session.request(op, '/'.join(baseUrl))
        elif op == 'POST':
            r = self.session.request(op, '/'.join(baseUrl), data=data, json=safeKeys(json))
        # check if successful
    ...
    def createStudy(self,x,find=False):
        ...
        try:
            panel = self.rest('panel',data={k: x[k] for k in ('panel_id', 'panel_version')})
            assert len(panel['data']) == 1
            assert set(x['subpanels']) <= set(map(lambda x: x['subpanel_id'], panel['data'][0]['subpanels']))
        ...
        try:
            study = self.rest('study',data={k: x[k] for k in ('study_name', 'group')})['data']
            assert len(study) == 0
        except AssertionError:
        ...
        # check sample
        try:
            sample = self.rest('sample',data={k: x[k] for k in ('sample_id','group')})
            ...
            for fi in files:
                ...
                if m:
                    ...
                    if os.path.isfile(fi) and m and filetype in ['vcf','bed','bedgraph','bam','pdf']:
                        ...
                        url += '?%s' % (urlencode({'filename': fi.split('/')[-1]})) # add filename
                        ...
                        print('ERROR: {} is not valid file'.format(os.path.basename(fi)))
                else:
                    print('ERROR: {} is an unsupported format'.format(os.path.basename(fi)))
            return results
...
if __name__=="__main__":
    ...
    with sqvd:
        ...
        print('STUDIES:',len(studies['data']),studies['userid'],studies['requested'],str(studies['querytime'])+'ms')
        ...
        # get document
        print('GET BY ID:',len(sqvd.rest('study','GET',study['data']['_id'])['data']))

        # with query parameters
        study = sqvd.rest('study',data={'study_name': study['data']['study_name']})
        print('GET BY NAME:',len(study['data']))

        # delete this study
        print('DELETE:',len(sqvd.rest('study','DELETE',study['data'][0]['_id'])['data']))
